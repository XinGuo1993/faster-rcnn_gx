...
class car_plate(datasets.imdb):
    ...
    def _load_selective_search_IJCV_roidb(self, gt_roidb):
        ...
        # return self.create_roidb_from_box_list(box_list, gt_roidb)
        pass
    def adaptive_padsize(self,x_min,y_min,x_max,y_max):
        width=x_max-x_min
        height=y_max-y_min
        pad_x=np.round(width*0.1)
        pad_y=np.round(height*0.2)
        return pad_x,pad_y
    def _load_plate_annotation(self, image_ids):
        ...
        for ind, keypoints in enumerate(all_keypoints):
            ...
            pad_x=35
            pad_y=35
            #pad_x,pad_y=adaptive_padsize(x_min,y_min,x_max,y_max)
            # dynamic scale parameters
            if self._source_name == 'baidu':
            ...
            boxes = np.array([x_min * x_scale - pad_x, y_min * y_scale - pad_y, 
                              x_max * x_scale + pad_x, y_max * y_scale + pad_y], 
